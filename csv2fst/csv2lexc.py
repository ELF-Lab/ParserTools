...
@click.option('--lexc-path', required=True, help="Directory where output lexc files are stored")
...
def main(config_file,lexc_path,read_lexical_database):
    ...
    print(f"Reading spreadsheets for irregular paradigms from directory: {config['source_path']}", file=stderr)
    irregular_lexicon = Lexicon(config, lexc_path, read_lexical_database=False, regular=False)
